...
import { Drag } from "@/api/status/status"
...
export default {
  ...
  mounted() {
    if(this.$store.getters.permissions.some(item => item==='*:*:*')
      ...
      {
        this.loading=true;
        Drag(this.$route.query.deviceId, this.$route.query.startDate, this.$route.query.endDate)
          .then(res => {
            let index1 = [];
            let index2 = [];
            this.averageVal1=0;
            this.averageVal2=0;
            this.abnormalProbability = res.abnormalProbability*100;
            for (let item of res.rows) {
              let temp = item.sendDate;
              let temp2 = item.tractorPitch;
              let temp3 = item.tractorRoll;
              index1.push(temp2);
              index2.push(temp3);
  ...
  methods: {
    init() {
      ...
      this.refresh = false;
      this.chartInstance = echarts.init(document.getElementById("drag"));
      this.chartInstance.setOption({
        title: {
          text: this.$t('deviceDetails.chart.drag_status.title'),
          left: "center",
          top: "1%",
          textStyle: {
            color: "#ffffff",
        ...
        series: [
          {
            name: this.$t('deviceDetails.chart.drag_status.pitch'),
            type: "line",
            symbol: 'none',
          ...
          {
            name: this.$t('deviceDetails.chart.drag_status.roll'),
            type: "line",
            symbol: 'none',
            itemStyle: {
              width: 1,
        ...
        graphic: [
          {
            ...
            style: {
              fill: '#ffffffb6',
              text: this.$t('deviceDetails.chart.drag_status.avg_pitch')
                + '：'
                + Number(this.averageVal1).toFixed(2)
                + '°  '
                + this.$t('deviceDetails.chart.drag_status.avg_roll')
                + '：'
                + Number(this.averageVal2).toFixed(2)
                + '°  '
                + this.$t('deviceDetails.chart.drag_status.pitch_probability')
                + '：'
                + Number(this.abnormalProbability).toFixed(2)
                + '%',

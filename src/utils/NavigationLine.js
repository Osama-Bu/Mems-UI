	...
	addData() {
		...
		this.datas.forEach(item => {
			let coords = item.map(data_ => {
				return fromLonLat([data_.lon, data_.lat], 'EPSG:4326')
			});
			// let coords = item.map(data_ => {
			// 	return fromLonLat([data_[0],data_[1]],'EPSG:4326')
			// });
			...
			if (dis_start2end > distance_) {//距离大于间隔
				...
				style_ = new Style({
					...
					image: new Icon({
						...
						scale: this.style.line_width / 180,
